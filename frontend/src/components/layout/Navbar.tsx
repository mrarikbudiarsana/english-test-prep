    ...
    <nav className="bg-white/90 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50 shadow-sm">
          ...
          <div className="hidden md:flex items-center gap-1">
            {user ? (
              <>
                ...
                <Link href="/tests" className={navLinkClass}>Practice Tests</Link>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-slate-200 bg-white/95 backdrop-blur-md px-4 py-3 space-y-1">
          {user ? (
            <>
              <Link href="/dashboard" className="block px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-[#e8f4fd] hover:text-[#08507f] rounded-xl" onClick={() => setMobileMenuOpen(false)}>Dashboard</Link>
              <Link href="/tests" className="block px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-[#e8f4fd] hover:text-[#08507f] rounded-xl" onClick={() => setMobileMenuOpen(false)}>Practice Tests</Link>
              <Link href="/pricing" className="block px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-[#e8f4fd] hover:text-[#08507f] rounded-xl" onClick={() => setMobileMenuOpen(false)}>Pricing</Link>
              {user.role === 'admin' && (
                <Link href="/admin" className="block px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-[#e8f4fd] hover:text-[#08507f] rounded-xl" onClick={() => setMobileMenuOpen(false)}>Admin Panel</Link>
              )}
              <Link href="/settings" className="block px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-[#e8f4fd] hover:text-[#08507f] rounded-xl" onClick={() => setMobileMenuOpen(false)}>Settings</Link>
              <div className="pt-2 border-t border-slate-100">
                <button onClick={() => { setMobileMenuOpen(false); logout(); }} className="block w-full text-left px-4 py-2.5 text-sm font-semibold text-red-600 hover:bg-red-50 rounded-xl">Sign Out</button>
              </div>
            </>
          ) : (
            <>
              <Link href="/login" className="block px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 rounded-xl" onClick={() => setMobileMenuOpen(false)}>Sign In</Link>
              <Link href="/register" className="block px-4 py-2.5 text-sm font-bold text-white text-center rounded-xl" style={{ background: 'linear-gradient(135deg, #08507f, #0a6aad)' }} onClick={() => setMobileMenuOpen(false)}>Get Started Free</Link>
            </>
          )}
        </div>
      )}
