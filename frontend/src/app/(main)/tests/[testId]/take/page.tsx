...
function VolumeControl({ volume, onChange }: { volume: number, onChange: (v: number) => void }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="relative w-32 h-1.5 bg-slate-100 rounded-full group cursor-pointer">
        <input 
          type="range"
          min="0"
          max="1"
        ...
        />
        {/* Track - No transition for smoothness */}
        <div 
          className="absolute left-0 top-0 h-full bg-blue-600 rounded-full"
          style={{ width: `${localVolume * 100}%` }}
        />
        {/* Thumb - No transition for smoothness */}
        <div 
          className="absolute top-1/2 -translate-y-1/2 h-3.5 w-3.5 bg-white border-2 border-blue-600 rounded-full shadow-md z-0"
          style={{ left: `calc(${localVolume * 100}% - 7px)` }}
        />
      </div>
      <span className="text-xs font-semibold text-slate-500 min-w-[2.5rem]">{Math.round(localVolume * 100)}%</span>
    </div>
  );
}
  ...
  return (
    <div className="h-screen flex flex-col bg-white overflow-hidden text-slate-800">
      ...
      <div className="flex-1 overflow-hidden flex">
        ...
        <main className="flex-1 overflow-y-auto bg-white border-r border-slate-50">
              ...
              <div className="w-full">
                {viewingDirections ? (
                  <div className={cn("w-full flex flex-col items-center justify-center min-h-[400px] transition-all duration-300", isDirectionsTransitioning ? "opacity-0 scale-[0.98]" : "opacity-100")}>
                    <div className="w-full max-w-5xl rounded-3xl border border-slate-100 bg-[#fbfcfd] p-10 sm:p-14 shadow-[0_10px_30px_rgba(0,0,0,0.03)] text-center">
                      <div className={cn('w-20 h-20 rounded-2xl flex items-center justify-center mb-10 mx-auto shadow-inner', toeflSectionTheme.tint, toeflSectionTheme.iconColor)}>
                        <ToeflSectionIcon className="w-10 h-10" />
                      </div>
                      ...
                      <button onClick={exitDirectionsView} className="rounded-xl bg-[#08507f] px-14 py-5 font-bold text-white text-lg hover:bg-[#064066] hover:shadow-xl hover:-translate-y-1 active:translate-y-0 transition-all uppercase tracking-widest">
               ...
               <div className="w-full">
                 {viewingDirections ? (
                  <div className="w-full flex flex-col items-center justify-center min-h-[400px]">
                    <div className="w-full max-w-3xl rounded-3xl border border-slate-100 bg-[#fbfcfd] p-10 sm:p-14 shadow-[0_10px_30px_rgba(0,0,0,0.03)] text-center">
                      <div className="w-20 h-20 bg-[#e8f4fd] text-[#08507f] rounded-2xl flex items-center justify-center mb-10 mx-auto shadow-inner"><HiBookOpen className="w-10 h-10" /></div>
                      <h2 className="text-4xl font-bold text-slate-900 mb-8 lowercase">reading comprehension</h2>
                      ...
                      <button onClick={exitDirectionsView} className="rounded-xl bg-[#08507f] px-14 py-5 font-bold text-white text-lg hover:bg-[#064066] hover:shadow-xl hover:-translate-y-1 active:translate-y-0 transition-all uppercase tracking-widest">Begin Reading Section</button>
                    </div>
                  </div>
                 ) : (
                  <div className="flex gap-10">
                    <div className={cn("flex-1", toeflReadingMobileView !== 'passage' && 'hidden lg:block')}>
                      <ReadingPassage title={currentSectionPart?.passageTitle || ''} content={currentSectionPart?.passageText || ''} highlightEnabled variant="toefl_itp" />
                    </div>
                    <div className={cn("flex-1", toeflReadingMobileView !== 'question' && 'hidden lg:block')}>
                      {currentQuestion && <QuestionRenderer question={currentQuestion} answer={state.answers[currentQuestion.id]} onAnswerChange={handleAnswerChange} displayNumber={currentQuestionIndex + 1} isActive />}
                    </div>
                  </div>
                 )}
               </div>
        ...
        <aside className="w-[320px] shrink-0 bg-[#f9fafb] p-6 overflow-y-auto flex flex-col border-l border-slate-100">
      ...
      {!viewingDirections && state.currentSectionType && (
        <footer className="shrink-0 border-t border-slate-100 bg-white px-8 py-4 z-40">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            <div className="flex items-center gap-4">
              <button 
                onClick={handlePreviousQuestion} 
                className="flex items-center gap-2 px-6 py-2.5 border-2 border-slate-100 rounded-lg text-sm font-bold text-slate-500 hover:bg-slate-50 hover:border-slate-200 transition-all disabled:opacity-30 disabled:cursor-not-allowed uppercase tracking-wider" 
              ...
              <button 
                ...
                className={cn(
                  "flex items-center gap-2 px-6 py-2.5 border-2 rounded-lg text-sm font-bold transition-all uppercase tracking-wider", 
            ...
            <button 
              ...
              className="flex items-center gap-3 px-10 py-2.5 bg-blue-600 text-white text-sm font-bold uppercase tracking-widest rounded-lg hover:bg-blue-700 hover:shadow-lg transition-all active:scale-95 group"
            >
